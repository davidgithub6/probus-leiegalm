...
export async function up({ db }: MigrateUpArgs): Promise<void> {
    ...
    await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD COLUMN \`verslagen_id\` integer;`)
    await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD COLUMN \`wie_is_wie_id\` integer;`)
    await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD COLUMN \`uitnodigingen_id\` integer;`)
